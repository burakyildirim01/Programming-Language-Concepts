...
;     #(struct:diff-exp #(struct:const-exp 55) #(struct:diff-exp #(struct:const-exp 22) #(struct:const-exp 11))))
  ...
  ;#(struct:a-program #(struct:let-exp fix1 #(struct:proc-exp f1 #(struct:let-exp d1 #(struct:proc-exp x1 #(struct:proc-exp z1 #(struct:call-exp #(struct:call-exp #(struct:var-exp f0) #(struct:call-exp #(struct:var-exp x0) #(struct:var-exp x0))) #(struct:var-exp z0)))) #(struct:proc-exp n1 #(struct:call-exp #(struct:call-exp #(struct:var-exp f0) #(struct:call-exp #(struct:var-exp d1) #(struct:var-exp d1))) #(struct:var-exp n0))))) #(struct:let-exp t4m1 #(struct:proc-exp f1 #(struct:proc-exp x1 #(struct:if-exp #(struct:zero?-exp #(struct:var-exp x0)) #(struct:const-exp 0) #(struct:diff-exp #(struct:call-exp #(struct:var-exp f0) #(struct:diff-exp #(struct:var-exp x0) #(struct:const-exp 1))) #(struct:const-exp -4))))) #(struct:let-exp times41 #(struct:call-exp #(struct:var-exp fix1) #(struct:var-exp t4m1)) #(struct:call-exp #(struct:var-exp times41) #(struct:const-exp 3))))))
  )